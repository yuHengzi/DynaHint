--
-- Patched TPC-DS referential-integrity script for the bundled
-- `tpcds.sql` schema used in this repo/environment.
--
-- Based on:
--   /home/houyuheng/download/DSGen-software-code-4.0.0/tools/tpcds_ri.sql
--
-- Local fixes applied:
-- 1. Removed invalid FK on catalog_page.cp_promo_id (column absent).
-- 2. Removed invalid FK on catalog_returns.cr_ship_date_sk (column absent).
-- 3. Removed invalid FK on customer.c_last_review_date_sk
--    because the schema uses c_last_review_date instead.
-- 4. Renamed duplicate constraint names:
--    - cr_i  -> cr_t   for catalog_returns.cr_returned_time_sk
--    - wr_ret_cd -> wr_ret_hd for web_returns.wr_returning_hdemo_sk
--    - ws_b_cd -> ws_b_hd for web_sales.ws_bill_hdemo_sk
--
